...
using namespace std;
	...
	/** Map object constructor. Conducts the creation, through file readin, of the node-based flightmap and array of flight requests that will be used in later operations.
		Will prompt user for appropriately formatted files.
	...
	@post Map object created with flight map and requested flights as specified in the data files.
	@return None. */
	...
	@post Flight map may have had a number of cities within it marked visited (these are reset to unvisited if this function is called again).
	...
	@pre Correctly read-in data from the constructor is needed.
	@post City returned; flight map is unchanged.
	...
	city map::getAdjacentCity ( const city source ) const
		{

		int index = 0;

		//creat temp city with name "NO_CITY" to return if needed
		...
		//set a pointer to the first destination from that city

		node* tempPtr = adjacencyMap[index]->next;

		//if that pointer is null there are no destinations, return a temp city with the name "NO_CITY"
		...
		while ( tempPtr->next != NULL )
			...
			//if an unvisited city is found, return it
		...
		//if not, return the temp city with name "NO_NAME"

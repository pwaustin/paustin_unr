        ...
        /* PROGRAM: project7.cpp (Card deal program with copy constructor and const functions - CS 202 Project 7)
        ...
        /* REVISIONS: revision of project6.cpp (Card deal program with classes - CS 202 Project 6)

        /*                                                                                       */

        /* PURPOSE: This program duplicates the shuffle and deal functionality of project 6, while expanding
		on the card and player classes from that program. This version of the program has added a copy
		constructor to the card class and has made member functions of the classes and function 
		parameters const wherever appropriate. 
...
int main()
{
	...
	while ( loopThroughMenu == true )
		...
		if ( userInput == '1' )
		   {
			...
			//conduct shuffling operation:
				...
				for ( index = 0; index < 52; index++ )
					...
					shuffledPointer[index].copy(unshuffledPointer[index]);
		...
		if ( userInput == '5' )
			...
			//as specified by the professor, here is some brief code to demonstrate that the copy and parameterized constructors work properly:
			...
			cout << "Card made with copy constructor:" << endl;
...
void shuffleCards ( card* shuffledPointer )
{
	...
	for ( index = 51; index > 1; index-- )
		...
		temp.copy(shuffledPointer[index]);

		shuffledPointer[index].copy(shuffledPointer[random]);

		shuffledPointer[random].copy(temp);
...
int dealCards ( card* shuffledPointer, player* playersPointer, const int numberOfPlayers )
{
	...
	for ( outerIndex = 0; outerIndex < 3; outerIndex ++ )
		...
		for ( innerIndex = 0; innerIndex < numberOfPlayers; innerIndex++ )
			...
			handPointer[outerIndex].copy(shuffledPointer[cardsDealt]);
...
int dealToDiscard ( card* shuffledPointer, card* discardPointer, int cardsDealt )
{
	...
	discardPointer[0].copy(shuffledPointer[cardsDealt]);
...
void dealToStock ( card* shuffledPointer, card* stockPointer, int cardsDealt )
{
	...
	for ( index = cardsDealt ; index < 52 ; index++ )
		...
		stockPointer[stockLocation].copy(shuffledPointer[index]);

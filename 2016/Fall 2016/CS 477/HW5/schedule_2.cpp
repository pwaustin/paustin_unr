...
void printOpt(char* solution, int size)		//recursively steps through the solution array, printing the optimal schedule
{

if ( size == 0 )
	{
	cout << "Week 0: " << solution[0] << '\t';	//base case: don't make another call, just print the value
	return;
	}
	...
	if (solution[size] == 'B')			//if B, print Bs for the remaining values (also a base case)
		{					

...
int ids(int start, int end)
{
	...
	int max_depth = vertexCount;	

	for ( int iterations = 0; iterations < max_depth; iterations++ )
		{
		...
		while ( !frontier.empty() )
			...
			for ( int j = 0; j != vertexCount; j++ )
				...
				if ( graph[u][j] > 0 && seen[j] == false )
					{

					int depthTrace = u;
					int depth = 0;

					while ( pred[depthTrace]!= -1 )
						{

						depthTrace = pred[depthTrace];
						depth++;
...
int main(int argc, char** argv)
{
	...
	shortestPath = ids(startCity, endCity);

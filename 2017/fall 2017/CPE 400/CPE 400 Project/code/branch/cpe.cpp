		...
		for ( int i = 0; i < graphSize; i++ )
			...
			if ( nodes[i].distToGateway != 0 )
				...
				for ( int j = 0; j < packetsToSend; j++ )
					...
					for ( int k = 0; k < nodes[i].neighborNodes.size(); k++ )
						{
						...
						if ( double( nodes[thisNeighbor].distToGateway / double( nodes[thisNeighbor].power / NODE_POWER ) ) < 
							double(nodes[dest].distToGateway / ( double(nodes[dest].power / NODE_POWER ) ) ) )
							{
							dest = thisNeighbor;
							}

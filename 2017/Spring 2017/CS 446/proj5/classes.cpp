/** CS 446 Project 5 - supporting data structures, class implementation
@file classes.cpp
...
bool Event::compPS2(const Event& src) const
	{
	if ( cycles < src.getCycles() )
		return true;
...
void Data::readConfigFile (char* targetPath)
	...
	for ( int i = 0; i < 18; i++ )
		...
		if ( line == "Version/Phase" )			//read in version/phase, checking for valid value
		...
		else if ( line == "Processor Quantum Number" )		//read in quantum number
			{
			fin >> quantumNumber;
			if ( quantumNumber <= 0 )
				{
				fin.close();
				throw "Error: invalid value for processor quantum, aborting.";
				return;
				}
			getline(fin,line);
			}

		else if ( line == "CPU Scheduling Code" )
			{
			...
			if ( line == "FCFS" )
				...
				{
				fin.close();
				throw "Error: invalid CPU scheduling code, aborting.";
				return;
				}
...
Helper function defining comparison for part 1 of priority scheduling: put all I and O events first

*/

bool compPS1(const Event& src1, const Event& src2 )
	{
	if ( src1.getCode() == 'I' | src1.getCode() == 'O' )
		{
		return true;
		}
...
Helper function defining comparison for part 2 of priority scheduling: used for descending order of events by cycles

*/

bool compPS2(const Event&src1, const Event& src2 )
	{
	if ( src1.getCycles() > src2.getCycles() )
		return true;
...
Helper function defining comparison for SJF scheduling: used for ascending order of events by cycles

*/

bool compSJF(const Event&src1, const Event& src2 )
	{
	if ( src1.getCycles() < src2.getCycles () )
		return true;
...
Data class schedule helper function, new in project 5. Takes the scheduling code read in from the config file and sorts the vector of events
to correspond to the specified scheduling algorithm. For FIFO, does nothing as the read in values will already be in FIFO order. For PS, sorts
the vector so that input and output operations will be first, in decreasing order of size. For SJF, sorts the vector so that events with the least
tasks will run first.
...
void Data::schedule()
	...
	if ( schedulingCode == 0 )	//no action necessary for FIFO- schedule in the order originally read in
		return;

	else if ( schedulingCode == 1 )	//for PS, sort vector so I and O processes in descending order of tasks come first
		{

		//stable sort the event sequence so I and O events come before all events except S start and A start, and order is preserved otherwise

		std::stable_sort (eventSequence.begin() + 2, eventSequence.end() - 2, compPS1 );

		//count the number of I and O events

		while ( eventSequence[numIO].getCode() == 'I' | eventSequence[numIO].getCode() == 'O' )
			numIO++;

		//sort only the I and O events into descending order
		//the largest I and O events will go first, thereby enforcing priority scheduling

		std::stable_sort (eventSequence.begin() + 2, eventSequence.begin() + numIO, compPS2 );

		return; //scheduling complete

		}

	else if ( schedulingCode == 2 ) //for SJF, sort vector so processes are in ascending order of tasks
		{

		//stable sort the event sequence for ascending order of number of tasks

		std::stable_sort (eventSequence.begin() + 2, eventSequence.end() - 2, compSJF );
...
void Data::simulate()
	...
	//schedule the events as specified by the CPU scheduling code

	schedule();

	//then run the simulation on the scheduled vector of events
